...
function normalizeAvatarUrl(raw) {
    ...
    if (!allowedPrefixes.some((prefix) => val.startsWith(prefix)))
        return '';
    return val;
}
...
function hasMeaningfulNickName(raw) {
    const nick = (0, nickName_1.normalizeNickName)(raw);
    if (!nick)
        return false;
...
function profileMetaNeedsMigration(user, normalizedMeta) {
    ...
    return (rawMeta.wechatAuthorized !== normalizedMeta.wechatAuthorized ||
...
function settingsNeedMigration(rawSettings, normalizedSettings) {
    ...
    return (rawSettings.theme !== normalizedSettings.theme ||
...
async function syncWechatProfile(openid, data) {
    ...
    if (typeof data.nickName === 'string') {
        ...
        if (rawNick) {
            const nick = (0, nickName_1.normalizeNickName)(rawNick);
            if (!nick)
                return fail('昵称不合法（最多 20 个字符）');
