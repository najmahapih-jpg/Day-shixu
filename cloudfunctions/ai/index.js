...
async function generateHabitInsight(_openid, _payload = {}) {
    ...
    const activeIdSet = new Set(activeHabits.map((habit) => habit._id).filter((id) => Boolean(id)));
    ...
    for (const item of checkInsData || []) {
        if (!item || !item.habitId || !item.date)
            continue;
        if (!activeIdSet.has(item.habitId))
            continue;
    ...
    try {
        switch (action) {
            case 'generateHabitInsight':
                return await generateHabitInsight(OPENID, data || {});
            default:
