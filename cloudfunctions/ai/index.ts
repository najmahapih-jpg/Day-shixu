...
type AiAction = 'generateHabitInsight'

type HabitInsightSnapshot = {
  activeHabits?: number
...
type GenerateHabitInsightPayload = {
  force?: boolean
...
type DayDoneMap = Map<string, Set<string>>
...
async function batchGet<T>(
  ...
  while (all.length < maxRecords) {
    const result = await collection.where(where).skip(skip).limit(PAGE).get()
...
function toDateStr(input?: string | Date | number): string {
...
function toIsoStr(input: string | Date | number = new Date()): string {
  const date = input ? new Date(input) : new Date()
  const utc8 = new Date(date.getTime() + 8 * 60 * 60 * 1000)
  const year = utc8.getUTCFullYear()
  const month = String(utc8.getUTCMonth() + 1).padStart(2, '0')
...
function getWeekday0to6(dateStr: string): number {
...
function shouldRunOnDate(habit: HabitDocument | null | undefined, dateStr: string): boolean {
  ...
  if (frequency === 'custom') {
    ...
    return customDays.some((day) => {
      if (day === 0) return weekday === 7
      return Number(day) === weekday
    })
  }
  return true
}

function getDayRate(dateStr: string, activeHabits: HabitDocument[], dayDoneMap: DayDoneMap): number {
  ...
  const doneSet = dayDoneMap.get(dateStr) || new Set<string>()
  let done = 0
  doneSet.forEach((id) => {
...
function avgRate(list: number[]): number {
...
function buildTrendAnalysis(direction: TrendDirection, delta: number): string {
  if (direction === 'up') {
...
function buildRecommendations(context: RecommendationContext): string[] {
  const list: string[] = []
...
function buildSlogans(context: RecommendationContext): string[] {
...
async function generateHabitInsight(_openid: string, _payload: GenerateHabitInsightPayload = {}): Promise<CloudResult<HabitInsightResult>> {
  ...
  const habitsData = await batchGet<HabitDocument>(
    habitsCol,
    { _openid: openid, isArchived: _.neq(true) },
    500,
  )
  const activeHabits = (habitsData || []).filter((habit) => !habit.isArchived)
  ...
  const activeIdSet = new Set(
    activeHabits.map((habit) => habit._id).filter((id): id is string => Boolean(id)),
  )
  const checkInsData = await batchGet<CheckInDocument>(
    checkInsCol,
    {
      _openid: openid,
  ...
  const dayDoneMap: DayDoneMap = new Map()
  for (const item of checkInsData || []) {
    ...
    const set = dayDoneMap.get(dateStr) || new Set<string>()
  ...
  const thisWeekRates = thisWeekDates.map((date) => getDayRate(date, activeHabits, dayDoneMap))
  const lastWeekRates = lastWeekDates.map((date) => getDayRate(date, activeHabits, dayDoneMap))
  ...
  const todayDoneSet = dayDoneMap.get(today) || new Set<string>()
  const todayDone = todayDoneSet.size
  const todayRate = todayTotal > 0 ? clampRate((todayDone / todayTotal) * 100) : 0

  let bestDay = ''
  let bestRate = -1
  thisWeekDates.forEach((dateStr, index) => {
    if (thisWeekRates[index] > bestRate) {
      bestRate = thisWeekRates[index]
...
export async function main(event: AiEvent = {}, _context?: unknown): Promise<CloudResult<HabitInsightResult>> {
  ...
  try {
    switch (action as AiAction) {
      case 'generateHabitInsight':
        return await generateHabitInsight(OPENID, (data as GenerateHabitInsightPayload | undefined) || {})
      default:
