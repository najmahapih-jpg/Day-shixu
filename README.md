...
- timeline ?????????????? owner ?????????????????????????????? legacy overlap / ritual / block ???
- ???????[`docs/TIMELINE_INDEX_HANDOFF.md`](docs/TIMELINE_INDEX_HANDOFF.md)
- ?????????`timeline.page-data-flow`?`timeline.calendar-shell`?`timeline.calendar-detail-contracts`?`timeline.calendar-view-contracts` ???? lane/date/scroll/layout/clock ???
...
| [时间轴拆分线交接](docs/TIMELINE_INDEX_HANDOFF.md) | timeline 1~12 阶段拆分的职责边界、测试保护与回归重点 |
