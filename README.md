...
| Node 版本 | `18.x`，来源于 `package.json` 与 `.nvmrc` |
| 质量门禁 | `npm.cmd run check:gate` 聚合类型检查、测试、预飞检查、仓库卫生和公开仓库安全检查 |
| 发布记录 | `releases/history/<environment>/` 存放 release / rollback manifest |
| 环境状态 | 版本控制内的默认配置只有 `dev` 为 `READY`；`staging` / `prod` 在公开配置中仍为 `UNCONFIGURED`，真实发布依赖本地 override 和微信后台资源 |
