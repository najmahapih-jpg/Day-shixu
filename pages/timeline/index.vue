<template>
  <HfPageBg variant="neutral" :showPattern="false" class="page page-transition" :class="{ 'page-entered': pageEntered, 'theme-neo': isNeoTheme }">
    ...
    <scroll-view
      ...
      :scroll-into-view="timelineAnchor"
      enhanced
      :bounces="false"
      scroll-with-animation
      @scroll="onScroll"
...
import { getHolidayInfo, getLunarDate, getSolarTerm, HOLIDAY_TYPE_LABEL, type HolidayType } from '@/utils/holiday'
...
// Ensure the page scrolls to now-line appropriately
onMounted(() => {
  if (isToday.value) {
    setTimeout(() => {
      const top = Math.max(0, nowLineTop.value - 300)
      uni.pageScrollTo({
        scrollTop: top,
        duration: 300
      })
...
const calendarDays = computed<CalendarDay[]>(() => {
  ...
  for (let i = firstDay - 1; i >= 0; i--) {
    ...
    days.push({
      ...
      holidayFull: '', holidayType: '', total: 0, completed: 0, rate: 0,
    })
  }
...
function selectDate(date: string) {
  ...
  loadDateData().then(() => {
    ...
    if (date === todayStr.value) {
      ...
      timelineAnchor.value = `h-${START_HOUR}`
    }
    triggerBlocksEntry()
  })
}
...
function scrollToCurrentTime() {
  const now = getBeijingDateParts()
  const maxVisibleHour = Math.max(START_HOUR, END_HOUR - 1)
  const targetHour = Math.max(START_HOUR, Math.min(maxVisibleHour, now.hour - 1))
  timelineAnchor.value = `h-${targetHour}`
}
...
.date-item {
  ...
  &__circle {
    ...
    border-radius: $radius-full;
    @include flex-center;
    transition: background $duration-normal $ease-out-soft, color $duration-normal $ease-out-soft;
  }
...
.month-nav-key {
  ...
  &:active {
    transform: translateY(8rpx); // Bottom-out the keyswitch
  ...
  &--left {
    ...
    &:active {
      background: #7E22CE; 
      color: $color-white;
      box-shadow: 0 0 0 $neutral-900, inset 0 4rpx 8rpx rgba(0,0,0,0.3); // Inner press indent
    }
  }
  ...
  &--right {
    ...
    &:active {
      background: #C2410C;
      color: $color-white;
      box-shadow: 0 0 0 $neutral-900, inset 0 4rpx 8rpx rgba(0,0,0,0.3);
    }
  }
...
.calendar-cell {
  ...
  &.is-selected {
    ...
    .dark-mode & { 
      box-shadow: 8rpx 8rpx 0 $brand-primary; 
      border-color: $brand-primary;
    }
  }
